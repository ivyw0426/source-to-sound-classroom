...
export function BootcampApplicationForm({
  ...
  if (openCohorts.length === 0) {
    return (
      <section className="rounded-[2rem] border border-forest-100 bg-white/90 p-6 text-center shadow-sm">
  ...
  return (
    <form action={formAction} className="grid gap-6">
      {state.message ? (
        <div
          className={`rounded-[2rem] p-4 text-sm font-medium leading-6 ${
      ...
      <button
        type="submit"
        disabled={isPending}
        className="focus-ring inline-flex min-h-11 items-center justify-center gap-2 rounded-full bg-water-700 px-5 py-3 text-sm font-semibold text-white hover:bg-water-900 disabled:cursor-wait disabled:bg-slate-400"
      >
...
function FormSection({
  ...
  return (
    <section className="rounded-[2rem] border border-forest-100 bg-white/90 p-5 shadow-sm sm:p-6">
  ...
  return (
    <label className="block">
      ...
      <textarea
        ...
        required={required}
        rows={4}
        className="focus-ring mt-2 w-full rounded-full border border-forest-100 px-3 py-3 text-sm text-slate-800"
      />
    </label>
  );
}
...
function Checkbox({
  ...
  return (
    <label className="flex gap-3 rounded-full border border-forest-100 bg-[#fbfcf4] p-3 text-sm leading-6 text-slate-700">
      <input
        type="checkbox"
        name={name}
        required
        className="mt-1 h-4 w-4 rounded border-forest-100 text-water-700 focus:ring-water-700"
      />
      <span>{children}</span>
    </label>
  );
